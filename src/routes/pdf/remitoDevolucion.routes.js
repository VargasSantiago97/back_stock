...
const addEncabezado = (doc, datos, copia) => {
    ...
    doc.fontSize(14).text('DEVOLUCIÓN DE REMITO', 350, 30);
...
router.get('/:id/:cant', async (req, res) => {
    ...
    try {
        const devolucion = await EgresoDevolucion.findOne({
            where: {
                id: remitoId
            }
        })
        ...
        if (!devolucion) {
            return res.status(404).json({ message: 'Devolucion de remito no encontrada' });
        }
