...
router.post('/ingresos/detalles', async (req, res) => {
...
router.post('/ingresos/datos', async (req, res) => {
...
router.post('/remitos', async (req, res) => {
    ...
    // Información del cliente
    worksheet.getCell('A5').value = 'LISTADO DE REMITOS DE EGRESO DE MERCADERÍA Y DEVOLUCIONES';
    worksheet.getCell('A5').font = { bold: true };
...
router.post('/remitos/detalles', async (req, res) => {
    ...
    // Información del cliente
    worksheet.getCell('K1').value = 'REMITOS DE EGRESO DE MERCADERÍA Y DEVOLUCIONES';
    worksheet.getCell('K2').value = 'Impreso el: ' + fechaHoy();
    worksheet.getCell('K3').value = 'Impreso por: ' + usuario;
...
router.post('/remitos/datos', async (req, res) => {
...
router.post('/operaciones', async (req, res) => {
    ...
    // Información del cliente
    worksheet.getCell('A5').value = 'LISTADO DE OPERACIONES REALIZADAS';
    worksheet.getCell('A5').font = { bold: true };
    ...
    // Agregar encabezado de la tabla
    worksheet.addRow(['Fecha', 'Tipo', 'Pto Vta', 'Numero', 'CLIENTE EGRESO', 'CUIT EGRESO', 'CLIENTE INGRESO', 'CUIT INGRESO', 'OBS', 'OBS SISTEMA']);
    const headerRow = worksheet.getRow(9);
    ...
    data.forEach(item => {
        worksheet.addRow([
            ...
            item.punto ? item.punto : '',
            item.numero ? item.numero : '',
            item.razon_social_egreso ? item.razon_social_egreso : '',
            item.cuit_egreso ? item.cuit_egreso : '',
            item.razon_social_ingreso ? item.razon_social_ingreso : '',
            item.cuit_ingreso ? item.cuit_ingreso : '',
            item.observaciones ? item.observaciones : '',
            item.observaciones_sistema ? item.observaciones_sistema : ''
        ]);
    });
...
router.post('/operaciones/detalles', async (req, res) => {
    ...
    const tableCellStyleNegativo = {
        ...
        fill: {
            type: 'pattern',
            pattern: 'solid',
            fgColor: { argb: 'FFFFCCCC' } // Color rojo claro en formato ARGB
        }
    };
    ...
    // Información del cliente
    worksheet.getCell('K1').value = 'OPERACIONES REALIZADAS';
    worksheet.getCell('K2').value = 'Impreso el: ' + fechaHoy();
    worksheet.getCell('K3').value = 'Impreso por: ' + usuario;
    ...
    for (const documento of data) {
        ...
        articulosAsociados.forEach(articulosAsociado => {
            const cantidad = articulosAsociado.ajuste == 'positivo' ? articulosAsociado.cantidad : articulosAsociado.cantidad * (-1)
            const cantidadUnidadFundamental = articulosAsociado.ajuste == 'positivo' ? articulosAsociado.cantidadUnidadFundamental : articulosAsociado.cantidadUnidadFundamental * (-1)
            ...
            if (articulosAsociado.ajuste == 'positivo') {
                ...
                row.eachCell((cell) => {
                    cell.style = tableCellStyleNegativo;
                });
...
router.post('/operaciones/datos', async (req, res) => {
